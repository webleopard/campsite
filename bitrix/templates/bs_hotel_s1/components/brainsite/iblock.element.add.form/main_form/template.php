...
while($ob = $res->GetNextElement())
{
	...
	if (is_array($arFields) && is_array($arProps))
		$bg_form=array_merge($arFields, $arProps);
			
}

$bg_css_form_str="";
$rsFile = CFile::GetByID($bg_form["PREVIEW_PICTURE"]);
$bgFile = $rsFile->Fetch();
if ($bgFile["SRC"]!="")
{
	$bg_css_form_str="background-image: url('".$bgFile["SRC"]."');";
		
	if ($bg_css_form_str!="")
	{
		$bg_css_form_str='style="'.$bg_css_form_str.'"';

	}
}

?>
<div class="main_form_container" <?=$bg_css_form_str ?>>
<div class="main_form <?=$GLOBALS["OPTION_PAGE_BLOCK_FORM_BG"] ? "bg_form":""?>">
<?
...
<?
$header=$arParams["FORM_HEADER"]!="" ? $arParams["FORM_HEADER"]:GetMessage("FORM_HEADER");
$comment=$arParams["FORM_COMMENT"]!="" ? $arParams["FORM_COMMENT"]:GetMessage("FORM_COMMENT");

?>

<h2 class="styled"><?=$header ?></h2>
<div class="descr"><?=$comment ?></div>
<div class="air p20"></div>	

	
<form name="iblock_add_<?=$arParams["AJAX_OPTION_ADDITIONAL"]?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
<div class="d-flex">
<div class="form <?=$GLOBALS["OPTION_PAGE_BLOCK_FORM_BG"] ? "col-12 col-md-7 pr-0 pl-0":"col-12"?>" >
	<?=bitrix_sessid_post()?>
	...
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
		...
		<div class="btn_wrapper"><input type="submit" class="btn" name="iblock_submit<?=$arParams['AJAX_OPTION_ADDITIONAL'] ?>" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" /></div>
