...
<?endif?>
<form name="iblock_add_<?=$arParams["AJAX_OPTION_ADDITIONAL"]?>" class="popup_form <?=(count($arResult["ERRORS"]) > 0 || !empty($arResult["MESSAGE"])) ? "hidden":"" ?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	<?=bitrix_sessid_post()?>
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
		<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
			...
			<input data-code="PRICE" class="hidden" name="PROPERTY[<?=$arResult["PRICE_ID"] ?>][0]" size="30" value="">			
			<?
		...
		<div class="clear"></div>
					<div class="text-center"><div class="btn_container ripple"><input type="submit" class="btn" name="iblock_submit<?=$arParams['AJAX_OPTION_ADDITIONAL'] ?>" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" /></div></div>
