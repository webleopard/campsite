		...
		case "iblock":
			if(array_key_exists($arItem["ITEM_ID"], $arResult["ELEMENTS"]))
			{
				...
				if ($arParams["SHOW_PREVIEW"] == "Y")
				{
					...
					elseif ($arElement["PROPERTY_IMAGES_VALUE"]>0)
					$arElement["PICTURE"] = CFile::ResizeImageGet($arElement["PROPERTY_IMAGES_VALUE"], array("width"=>$PREVIEW_WIDTH, "height"=>$PREVIEW_HEIGHT), BX_RESIZE_IMAGE_PROPORTIONAL, true);
					
					elseif (is_array($arElement["PROPERTY_IMAGES_VALUE"]))
					$arElement["PICTURE"] = CFile::ResizeImageGet($arElement["PROPERTY_IMAGES_VALUE"][0], array("width"=>$PREVIEW_WIDTH, "height"=>$PREVIEW_HEIGHT), BX_RESIZE_IMAGE_PROPORTIONAL, true);
