	...
	var slider_<?=$block_id?>_<?=$CODE?>=$("#slider_<?=$block_id?>_<?=$CODE?>").slider({
		...
		slide: function( event, ui ) 
		{
			var val1 = (ui.values[ 0 ]+'').replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ');
    	  	...
    	  	$(blockCatalogSelectorModulAdmin+' .slider_data_<?=$CODE?> .slider_from').val(val1);
    	  	$(blockCatalogSelectorModulAdmin+' .slider_data_<?=$CODE?> .slider_to').val(val2);

    		$(blockCatalogSelectorModulAdmin+' [name=<?=$CODE?>_from]').val(ui.values[0]);
    		$(blockCatalogSelectorModulAdmin+' [name=<?=$CODE?>_to]').val(ui.values[1]);


		},
		stop: function( event, ui ) 
		{
			$(blockCatalogSelectorModulAdmin+' [name=<?=$CODE?>_from]').trigger("change");	
		},
